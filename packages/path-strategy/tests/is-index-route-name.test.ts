...
describe("isIndexRouteName", () => {
  describe("base name (index or empty)", () => {
    test('returns true for "index"', () => {
      const result = {
        noOpts: isIndexRouteName("index"),
        withLocaleCodes: isIndexRouteName("index", { localeCodes }),
      };
    ...
    test("returns true for empty string", () => {
      const result = {
        noOpts: isIndexRouteName(""),
        withLocaleCodes: isIndexRouteName("", { localeCodes }),
      };
      expect(result).toMatchSnapshot();
    });
  ...
  describe("localized index name (localized-index-{locale})", () => {
    ...
    test("returns false for localized-index-{locale} when locale not in localeCodes", () => {
      const result = {
        fr: isIndexRouteName("localized-index-fr", { localeCodes }),
        xx: isIndexRouteName("localized-index-xx", { localeCodes }),
      };
      expect(result).toMatchSnapshot();
    });
    ...
    test("custom prefix", () => {
      const result = {
        i18nIndexDe: isIndexRouteName("i18n-index-de", {
          localizedRouteNamePrefix: "i18n-",
          localeCodes: ["de"],
        }),
        localizedIndexDe: isIndexRouteName("localized-index-de", {
          localizedRouteNamePrefix: "i18n-",
          localeCodes: ["de"],
        }),
      };
