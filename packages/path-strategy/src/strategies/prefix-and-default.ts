import { buildUrl, hasKeys, joinUrl, transformNameKeyToPath } from "@i18n-micro/utils";
import {
...
export class PrefixAndDefaultPathStrategy extends BasePathStrategy {
  ...
  resolveLocaleRoute(
    ...
    if (inputName && hasParams) {
      ...
      if (routeWithParams !== null) {
        return this.applyBaseUrl(
          targetLocale,
          this.rewriteWithLocalePrefix(routeWithParams, targetLocale),
        );
      }
    }
    ...
    const baseName =
      this.getRouteBaseName(resolved) ?? inputName ?? resolved.name?.toString() ?? null;
    ...
    if (hasGR) {
      ...
      if (resolved.path && resolved.path !== "/" && resolved.name) {
        ...
        if (pathWithoutLocale && pathWithoutLocale !== "/") {
          return this.applyBaseUrl(
            targetLocale,
            joinWithLocalePrefix(pathWithoutLocale, targetLocale),
          );
        }
      }
    }
    ...
    if (inputName && !hasParams) {
      ...
      if (
        ...
        routeByLocalizedName = tryResolveByLocalizedName(
          this.ctx.router,
          prefix,
          baseName,
          targetLocale,
      ...
      if (routeByLocalizedName !== null) {
        return this.applyBaseUrl(
          targetLocale,
          this.rewriteWithLocalePrefix(routeByLocalizedName, targetLocale),
        );
      }
    }
