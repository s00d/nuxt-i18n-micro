...
export function useLocaleHead(options: UseLocaleHeadOptions = {}) {
  ...
  function updateMeta(canonicalQueryWhitelist: string[] = []) {
    ...
    const pathSegments = getPathSegments(fullPath);
    const matchedLocale = allLocales.find(
      (localeItem: Locale) => pathSegments[0] === localeItem.code,
    );
    ...
    const alternateLinks: MetaLink[] = alternateLocales.flatMap((loc: Locale) => {
      ...
      const switchedPath =
        typeof switchedPathResult === "string"
          ? switchedPathResult
          : switchedPathResult?.path || "/";
      if (!switchedPath) {
        return [];
      }
    ...
    const defaultLocObj = injectedDefaultLocale
      ? allLocales.find((l: Locale) => l.code === injectedDefaultLocale)
      : undefined;
    if (
      i18nLocaleRoute &&
      routerStrategy &&
      injectedDefaultLocale &&
      defaultLocObj?.seo !== false
    ) {
      const currentPath = routerStrategy.getCurrentPath();
      const defaultPathResult = i18nLocaleRoute(currentPath, injectedDefaultLocale);
      const defaultPath =
        typeof defaultPathResult === "string" ? defaultPathResult : defaultPathResult?.path || "/";
      if (defaultPath) {
