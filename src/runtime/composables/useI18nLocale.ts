...
export function useI18nLocale() {
  ...
  const getPreferredLocale = (): string | null => {
    const preferred =
      localeState.value ?? (i18nConfig.hashMode ? hashCookie.value : localeCookie.value);
    return preferred && validLocales.includes(preferred) ? preferred : null;
  };

  const getLocale = (): string | null => {
    return (
      localeState.value ?? (i18nConfig.hashMode ? hashCookie.value : localeCookie.value) ?? null
    );
  };
