...
testPrefixExceptDefault.describe("pages: false with i18n (prefix_except_default)", () => {
  ...
  testPrefixExceptDefault(
    "prefix_except_default: renders default locale content on root path",
    async ({ page, goto }) => {
  ...
  testPrefixExceptDefault(
    "prefix_except_default: respects cookie locale and shows correct translations",
    async ({ page, goto, baseURL }) => {
  ...
  testPrefixExceptDefault(
    "prefix_except_default: handles invalid cookie gracefully",
    async ({ page, goto, baseURL }) => {
  ...
  testPrefixExceptDefault(
    "prefix_except_default: handles cleared cookie gracefully",
    async ({ page, goto }) => {
      // No cookie set
      await goto("/", { waitUntil: "hydration" });
...
testNoPrefix.describe("pages: false with i18n (no_prefix)", () => {
  ...
  testNoPrefix(
    "no_prefix: respects cookie locale and shows correct translations",
    async ({ page, goto, baseURL }) => {
      // Set Chinese locale cookie
      await page.context().addCookies([
        {
