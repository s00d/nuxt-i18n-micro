import { extractBaseRoutePattern, routeNameToPath, stripLeadingSlash, stripLocalizedRouteNamePrefix } from './route-pattern'
...
export function findAllowedLocalesForRoute(
  ...
  const normalizedRouteName = stripLocalizedRouteNamePrefix(routeName, localizedRouteNamePrefix)
  const normalizedRoutePath = routeNameToPath(normalizedRouteName)
  ...
  let allowedLocales =
    ...
    (normalizedRoutePath && routeLocales?.[stripLeadingSlash(normalizedRoutePath)]) ||
    routeLocales?.[routePath] ||
    (routePath && routeLocales?.[stripLeadingSlash(routePath)])
  ...
  if (!allowedLocales && routeLocales && localeCodes?.length) {
    ...
    if (first && localeCodes.includes(first) && segments.length > 1) {
      ...
      const pathKey = pathWithoutLocale === '/' ? '/' : stripLeadingSlash(pathWithoutLocale)
