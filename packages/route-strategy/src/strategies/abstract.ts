...
export abstract class BaseStrategy implements RouteStrategy {
  processPage(page: NuxtPage, context: GeneratorContext): NuxtPage[] {
    ...
    const isLocalizationDisabled =
      context.globalLocaleRoutes[pageName] === false ||
  ...
  protected localizeChildrenAllLocales(
    ...
    return children.flatMap((child) =>
      this.localizeChildAllLocales(child, parentPath, parentOriginalPath, localeCodes, context),
    );
  }
  ...
  private localizeChildAllLocales(
    ...
    if (customLocalePaths) {
      return localeCodes.flatMap((locale) => {
        const parentLocalizedPath =
          customLocalePaths[locale] ?? resolveChildPath(parentPath, child.path ?? "");
