...
function measureBundleSize(directory: string): BundleSize {
  ...
  function isTranslationFile(filePath: string): boolean {
    ...
    if (relativePath.includes("chunks/_") || relativePath.includes("chunks\\_")) {
      const localePattern =
        /^(?:en|de|ru|fr|es|it|pt|zh|ja|ko|ar|he|hi|pl|nl|sv|da|fi|no|cs|sk|hu|ro|bg|uk|tr|vi|th|id|ms)\.mjs$/i;
    ...
    if (relativePath.includes("chunks/build") || relativePath.includes("chunks\\build")) {
      const localePattern =
        /^(?:en|de|ru|fr|es|it|pt|zh|ja|ko|ar|he|hi|pl|nl|sv|da|fi|no|cs|sk|hu|ro|bg|uk|tr|vi|th|id|ms)-\w+\.mjs$/i;
      if (localePattern.test(fileName)) {
        return true;
      }
    }
    ...
    if (relativePath.includes("chunks/raw") || relativePath.includes("chunks\\raw")) {
      const localePattern =
        /^(?:en|de|ru|fr|es|it|pt|zh|ja|ko|ar|he|hi|pl|nl|sv|da|fi|no|cs|sk|hu|ro|bg|uk|tr|vi|th|id|ms)\d*\.mjs$/i;
      if (localePattern.test(fileName)) {
        return true;
      }
    }
    ...
    if (
      relativePath.includes("routes/api/translations") ||
      relativePath.includes("routes\\api\\translations")
    ) {
      return true;
    }
  ...
  function getDirSizeWithSeparation(dir: string): {
    total: number;
    code: number;
    translations: number;
  } {
...
function extractChartData(artillery: ArtilleryResult): ChartDataPoint[] {
  ...
  return intermediate.map((entry, index) => ({
    ...
    vusersActive:
      (entry.counters["vusers.created"] || 0) -
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          backgroundColor: chartColors.responseTimeP95
            .replace("rgb", "rgba")
            .replace(")", ", 0.1)"),
          borderWidth: 2,
          tension: 0.3,
...
function generateChartMarkdown(name: string, artillery: ArtilleryResult): string {
  ...
  const completedPercent =
    summary.vusersCreated > 0
      ? ((summary.completed / summary.vusersCreated) * 100).toFixed(2)
      : "0";
  const failedPercent =
    summary.vusersCreated > 0 ? ((summary.failed / summary.vusersCreated) * 100).toFixed(2) : "0";
      ...
      datasets: [
        {
          label: "Requests per Second",
          data: results.map((r) =>
            Math.round(r.artillery?.aggregate.rates["http.request_rate"] || 0),
          ),
          backgroundColor: [chartColors.plainNuxt, chartColors.i18nV10, chartColors.i18nMicro],
          borderColor: [chartColors.plainNuxt, chartColors.i18nV10, chartColors.i18nMicro],
          borderWidth: 2,
...
function generateComparisonMarkdown(
  ...
  const rpsWinner = results.reduce((best, curr) =>
    (curr.autocannon?.requests.average || 0) > (best.autocannon?.requests.average || 0)
      ? curr
      : best,
  );
  const latencyWinner = results.reduce((best, curr) =>
    (curr.autocannon?.latency.average || Infinity) < (best.autocannon?.latency.average || Infinity)
      ? curr
      : best,
  );
...
async function generateAndSaveChart(name: string, artillery: ArtilleryResult): Promise<string> {
  ...
  fs.writeFileSync(
    jsonPath,
    JSON.stringify({ data, summary, intermediate: artillery.intermediate }, null, 2),
  );
...
async function stressTestServerWithArtillery(
  ...
  childProcess.stdout?.on("data", (data: Buffer) =>
    console.log(`[Server stdout ${name}]: ${data.toString().trim()}`),
  );
  childProcess.stderr?.on("data", (data: Buffer) =>
    console.error(`[Server stderr ${name}]: ${data.toString().trim()}`),
  );
...
function logAndWriteComparisonResults(
  name1: string,
  name2: string,
  ...
  const comparisons = {
    ...
    requestsPerSecondDifference:
      (result2.requestsPerSecond || 0) - (result1.requestsPerSecond || 0),
    autocannonRpsDifference:
      (result2.autocannon?.requests.average || 0) - (result1.autocannon?.requests.average || 0),
    autocannonLatencyDifference:
      (result2.autocannon?.latency.average || 0) - (result1.autocannon?.latency.average || 0),
  };
...
describe("performance", () => {
  it(
    ...
    async () => {
      ...
      expect(plainNuxtResults.buildTime).toBeGreaterThan(0);
      await pause(5000);
      ...
      console.log(
        `i18n v10: ${i18nResults.buildTime.toFixed(2)} seconds, Bundle: ${formatBytes(i18nResults.bundleSize?.total || 0)}`,
      );
      console.log(
        `i18n-micro: ${i18nMicroResults.buildTime.toFixed(2)} seconds, Bundle: ${formatBytes(i18nMicroResults.bundleSize?.total || 0)}`,
      );
      ...
      const { buildTimeConfig, bundleSizeConfig } = generateBuildComparisonCharts(
        buildTimes,
        codeBundleSizesMB,
        translationSizesMB,
      );
...
`);
      ...
      // 2. Stress tests
      const plainNuxtStressResults = await stressTestServerWithArtillery(
        "./test/fixtures/plain-nuxt",
        "plain-nuxt",
        artilleryConfigPath,
      );
      await pause(5000);
      const i18nStressResults = await stressTestServerWithArtillery(
        "./test/fixtures/i18n",
        "i18n",
        artilleryConfigPath,
      );
      await pause(5000);
      const i18nMicroStressResults = await stressTestServerWithArtillery(
        "./test/fixtures/i18n-micro",
        "i18n-micro",
        artilleryConfigPath,
      );
      ...
      const comparisonResults = [
        {
          name: "plain-nuxt",
          autocannon: plainNuxtStressResults.autocannon,
          artillery: plainNuxtStressResults.artillery,
        },
        {
          name: "i18n-v10",
          autocannon: i18nStressResults.autocannon,
          artillery: i18nStressResults.artillery,
        },
        {
          name: "i18n-micro",
          autocannon: i18nMicroStressResults.autocannon,
          artillery: i18nMicroStressResults.artillery,
        },
      ];
      const { rpsConfig, latencyConfig, artilleryRpsConfig } =
        generateComparisonCharts(comparisonResults);
...
`);
      ...
      // 4. Comparisons
      logAndWriteComparisonResults(
        "plain-nuxt (baseline)",
        "i18n v10",
        plainNuxtStressResults,
        i18nStressResults,
      );
      logAndWriteComparisonResults(
        "plain-nuxt (baseline)",
