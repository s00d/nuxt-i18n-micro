...
import type { PathStrategyContext, ResolvedRouteLike, RouteLike } from "../src/types";
...
function printReport(title: string, results: BenchResult[], baseline: Baseline | null): void {
  ...
  if (hasBaseline) {
    lines.push(
      row(
        `${pad("Function", NW)}${padL("ops/ms", CW)}${padL("prev", CW)}${padL("delta", CW)}   `,
        W,
      ),
    );
  } else {
    lines.push(
      row(`${pad("Function", NW)}${padL("ops/ms", CW)}${padL("ms", CW)}${" ".repeat(CW + 3)}`, W),
    );
  }
  lines.push(`\u2551${"\u2500".repeat(W - 2)}\u2551`);
  ...
  for (const r of results) {
    ...
    if (hasBaseline && prev) {
      ...
      lines.push(
        row(
          `${pad(r.name, NW)}${padL(r.opsPerMs.toFixed(1), CW)}${padL(prev.opsPerMs.toFixed(1), CW)}${padL(delta, CW)} ${flag}`,
          W,
        ),
      );
    } else {
      lines.push(
        row(
          `${pad(r.name, NW)}${padL(r.opsPerMs.toFixed(1), CW)}${padL(r.totalMs.toFixed(2), CW)}${" ".repeat(CW + 3)}`,
          W,
        ),
      );
    }
  ...
  for (const [name, fn] of cases) {
    test(`${name}`, () => {
      ...
      if (prev) {
        ...
        if (ratio <= 1 - threshold) {
          throw new Error(
            `"${name}" regressed by ${((1 - ratio) * 100).toFixed(1)}% vs baseline (threshold ${(threshold * 100).toFixed(0)}%)`,
          );
        }
      }
      const minOps = prev ? 0 : 1;
      expect(r.opsPerMs).toBeGreaterThan(minOps);
    });
  }
});
  ...
  return {
    ...
    router: {
      ...
      hasRoute: (name: string) => {
        return (
          routeMap.has(name) ||
          routeMap.has(name.replace(/^localized-/, "").replace(/-(?:en|de|fr)$/, ""))
        );
      },
    },
  };
...
afterAll(() => {
  ...
  if (baseline) {
    summary.push(
      `  Baseline: ${Object.keys(baseline).length} entries | Threshold: -${REGRESSION_THRESHOLD * 100}%`,
    );
  } else {
