...
export const I18nT: Component<I18nTProps> = (props): JSX.Element => {
  ...
  const translation = createMemo(() => {
    ...
    if (local.number !== undefined) {
      ...
      const result = i18n.t(
        local.keypath,
        { number: formattedNumber, ...(local.params || {}) },
        undefined,
        route,
      );
    ...
    if (local.date !== undefined) {
      ...
      const result = i18n.t(
        local.keypath,
        { date: formattedDate, ...(local.params || {}) },
        undefined,
        route,
      );
      return typeof result === "string" ? result : String(result);
    ...
    if (local.relativeDate !== undefined) {
      ...
      const result = i18n.t(
        local.keypath,
        { relativeDate: formattedRelativeDate, ...(local.params || {}) },
        undefined,
        route,
      );
      return typeof result === "string" ? result : String(result);
    ...
    const translationResult = (
      i18n.t(local.keypath, local.params, local.defaultValue, route) ?? ""
    ).toString();
  ...
  if (local.hideIfEmpty && !translationValue.trim()) {
    if (local.defaultValue) {
      return (
        <Dynamic component={local.tag || "span"} {...(others as Record<string, unknown>)}>
          {local.defaultValue}
        </Dynamic>
      );
    }
