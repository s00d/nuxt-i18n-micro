...
test.describe("Critical i18n scenarios", () => {
  ...
  test("reactivity: computed translations update immediately after locale switch", async ({
    page,
    goto,
  }) => {
    ...
    // 5. Переходим на /page (дефолтный маршрут) — локаль меняется на defaultLocale (en)
  ...
  test("reactivity: computed with $ts updates on locale switch without $getLocaleName workaround", async ({
    page,
    goto,
  }) => {
    await goto("/page", { waitUntil: "hydration" });
  ...
  test("performance: does not load translations for other locales on initial load", async ({
    page,
    goto,
  }) => {
    ...
    const _deRequests = requestedUrls.filter(
      (url) => url.includes("/de/") && !url.includes("/general/"),
    );
    const _ruRequests = requestedUrls.filter(
      (url) => url.includes("/ru/") && !url.includes("/general/"),
    );
  ...
  test("routing: switchLocaleRoute preserves complex query and hash", async ({ page, goto }) => {
    ...
    await goto("/news/1?category=tech&sort=date&page=2&filter=active#comments", {
      waitUntil: "hydration",
    });
  ...
  test("merge precedence: page translations override global translations", async ({
    page,
    goto,
  }) => {
