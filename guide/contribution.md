---
url: 'https://s00d.github.io/nuxt-i18n-micro/guide/contribution.md'
...
Packages are versioned independently and **by hand** — `changelogen` only bumps the root package. Since `pnpm publish -r` silently *skips* any package whose version already exists on npm, a forgotten bump does not fail the release: it just leaves that package stale on npm while everything else moves on.
