...
import { type GlobalLocaleRoutes, type Locale, type Strategies } from "@i18n-micro/types";
import { normalizeRouteKey } from "@i18n-micro/utils";
import { type NuxtPage } from "@nuxt/schema";
import { type LocaleRoutesConfig } from "../strategies/types";
import {
  type LocalizedPathsMap,
  extractLocalizedPaths,
  pathKeyForLocalizedPaths,
...
export class GeneratorContext {
  ...
  constructor(options: GeneratorContextOptions) {
    ...
    this.defaultLocale = this.findLocaleByCode(options.locales, options.defaultLocaleCode) ?? {
      code: options.defaultLocaleCode,
    };
