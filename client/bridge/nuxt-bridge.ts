 ...
 */
export function createNuxtBridge(client: {
  devtools: {
    extendClientRpc: (
      namespace: string,
      handlers: { localesUpdated: (_updatedLocales: LocaleData) => void },
    ) => {
      getLocalesAndTranslations: () => Promise<LocaleData>;
      getConfigs: () => Promise<ModuleOptions>;
      saveTranslationContent: (filePath: string, content: TranslationContent) => Promise<void>;
      localesUpdated?: (data: LocaleData) => void;
    };
  };
}): I18nDevToolsBridge {
  ...
  return {
    async getLocalesAndTranslations(): Promise<LocaleData> {
      ...
      try {
        ...
        console.log("[i18n-devtools] Bridge: getLocalesAndTranslations result", {
