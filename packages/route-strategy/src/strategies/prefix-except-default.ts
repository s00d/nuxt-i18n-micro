...
export class PrefixExceptDefaultStrategy extends BaseStrategy {
  protected generateVariants(page: NuxtPage, context: GeneratorContext): NuxtPage[] {
    ...
    // 1. Default locale route: a single non-prefixed entry (like defaultRoute in the legacy RouteGenerator)
    if (allowedLocales.includes(defaultLocaleCode)) {
      ...
      const localizedParentPaths = defaultCustomPath
        ? { [defaultLocaleCode]: defaultCustomPath }
        : {};
    ...
    // 3. Aliases: same behavior as legacy code — for all allowed locales (including default)
  ...
  override postProcess(pages: NuxtPage[], context: GeneratorContext): NuxtPage[] {
    // Group base and localized routes: base first, then localized-* ones
    ...
    const result = [...basePages, ...localizedPages];
