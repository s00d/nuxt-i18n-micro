...
describe('@i18n-micro/route-strategy dist publish smoke', () => {
  it('loads root entry (esm + cjs)', async () => {
    const mods = await smokeLoadExports(packageRoot, pkg, [{ subpath: '.', formats: ['esm', 'cjs'] }])
    expect(getLoadedModule(mods, '.:import').RouteGenerator).toBeTypeOf('function')
    expect(getLoadedModule(mods, '.:require').RouteGenerator).toBeTypeOf('function')
    expect(getLoadedModule(mods, '.:import').normalizePath).toBeTypeOf('function')
  })
})
