...
const _currentRouteWithQuery: ResolvedRouteLike = {
  name: "localized-about-en",
  path: "/about",
...
function snapshotState(strategyName: StrategyName) {
  ...
  for (const loc of targetLocales) {
    localeRouteByName[loc] = strategy.localeRoute(
      loc,
...
describe("strategies deep snapshots — defaultLocale and two locales only", () => {
  ...
  test("prefix_except_default with two locales: state", () => {
    ...
    const state = {
      ...
      canonicalResult: {
        en: strategy.getCanonicalPath(currentRouteAbout, "en"),
        de: strategy.getCanonicalPath(currentRouteAbout, "de"),
      },
    };
