...
function buildCacheEntry(data: Record<string, unknown>): {
...
export default defineNitroPlugin((nitroApp: NitroApp) => {
  ...
  function mergeAndSet(serverCache: CacheLike, locale: string, pageName: string): void {
    ...
    const aliases = Object.keys(routesLocaleLinks).filter(
      (alias) => routesLocaleLinks[alias] === pageName,
    );
    ...
    try {
      if (isPageTranslation) {
        ...
        if (!configuredLocales.has(locale)) {
          warn(
            `Detected ${event} for '${relativePath}', but locale '${locale}' is not in i18n.locales. Update config and restart dev server.`,
          );
          return;
        }
