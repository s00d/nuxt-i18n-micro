/**
 * Shared path and URL helpers used across packages.
 * These utilities are intentionally framework-agnostic.
 */
...
export function cleanDoubleSlashes(p: string): string {
  ...
  if (protoIdx === -1) return dedupeSlashes(p);
  return p.slice(0, protoIdx + 3) + dedupeSlashes(p.slice(protoIdx + 3));
}

function dedupeSlashes(s: string): string {
...
export function joinUrl(...segments: (string | undefined | null)[]): string {
  if (segments.length === 2) {
    const a = segments[0];
    const b = segments[1];
    if (a && b) return joinTwo(a, b);
    if (a) return normSimple(a);
    if (b) return normSimple(b);
    return "/";
  }
...
function joinTwo(a: string, b: string): string {
  ...
  if (result.indexOf("://") !== -1) {
    if (result.length > 1 && result.charCodeAt(result.length - 1) === 47)
...
export function getPathWithoutLocale(
...
): { pathWithoutLocale: string; localeFromPath: string | null } {
  ...
  if (firstSegment && localeCodes.includes(firstSegment)) {
    const remaining = normalized.slice(1 + firstSegment.length);
    const result =
      !remaining || remaining === "/"
        ? "/"
        : remaining.charCodeAt(0) === 47
          ? remaining
          : `/${remaining}`;
...
export function getLocaleFromPath(path: string, localeCodes: readonly string[]): string | null {
  ...
  if (segment && localeCodes.includes(segment)) return segment;
  return null;
}
