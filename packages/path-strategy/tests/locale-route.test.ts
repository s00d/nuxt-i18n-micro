...
describe("localeRoute - no_prefix", () => {
  ...
  test('{ name: "index" } returns root path /', () => {
    ...
    const result = {
      en: strategy.localeRoute("en", { name: "index" }),
      de: strategy.localeRoute("de", { name: "index" }),
    };
  ...
  test("object with name: returns route with localized name when router has it", () => {
    ...
    const strategy = createPathStrategy(makeCtx("no_prefix", { router }));
    const result = strategy.localeRoute(
      "de",
      { name: "localized-about-en", path: "/about" },
      currentRoute,
    );
...
describe("localeRoute - prefix_and_default", () => {
  ...
  test("with globalLocaleRoutes: custom path with prefix", () => {
    const strategy = createPathStrategy(
      makeCtx("prefix_and_default", { globalLocaleRoutes: { "/about": { de: "/ueber-uns" } } }),
    );
  ...
  test("with globalLocaleRoutes unlocalized: false returns path without locale prefix", () => {
    const strategy = createPathStrategy(
      makeCtx("prefix_and_default", {
        globalLocaleRoutes: {
          unlocalized: false,
          page2: { en: "/custom-page2-en", de: "/custom-page2-de" },
        },
      }),
    );
    const result = {
      en: strategy.localeRoute("en", { name: "unlocalized" }),
      de: strategy.localeRoute("de", { name: "unlocalized" }),
    };
    expect(result).toMatchSnapshot();
  });
});
...
describe("snapshots (documentation: how localeRoute works per strategy)", () => {
  ...
  test("localeRoute: object with name and path returns RouteLike with path/fullPath", () => {
    ...
    const resultEn = strategy.localeRoute(
      "en",
      { name: "localized-about-en", path: "/about" },
      currentRoute,
    );
    const resultDe = strategy.localeRoute(
      "de",
      { name: "localized-about-en", path: "/about" },
      currentRoute,
    );
