...
export function defaultSwitchLocaleRoute(
  ...
  if (found) {
    ...
    return s.getSwitchLocaleFallbackWhenNoRoute(
      route,
      s.buildLocalizedRouteName(baseName, toLocale),
    );
  }
...
function buildPathFromBaseNameAndParams(
  s: DefaultImplHost,
  baseName: string,
  ...
  if (paramKeys.length === 1 && firstKey !== undefined && baseName.endsWith(`-${firstKey}`)) {
    pathTemplate = joinUrl(
      "/",
      `${baseName.slice(0, baseName.length - firstKey.length - 1)}-:${firstKey}`,
    );
  } else {
...
export function defaultResolveLocaleRoute(
  ...
  if (inputName && hasParams) {
    ...
    if (routeWithParams !== null)
      return preserveQueryAndHash(s.applyBaseUrl(targetLocale, routeWithParams), src);
  }

  if (inputName && !hasParams) {
    let routeByLocalizedName = tryResolveByLocalizedName(
      router,
      prefix,
      inputName,
      targetLocale,
    ...
    if (routeByLocalizedName !== null)
      return preserveQueryAndHash(s.applyBaseUrl(targetLocale, routeByLocalizedName), src);
  }
  ...
  if (hasGR) {
    ...
    if (!hasParams && resolved.path && resolved.path !== "/" && resolved.name) {
      ...
      if (pathWithoutLocale && pathWithoutLocale !== "/") {
        const pathToUse =
          baseRouteName && pathWithoutLocale === baseRouteName
  ...
  const baseNameForPath =
    resolvedNameStr === inputName ? effectiveBaseName : (inputName ?? effectiveBaseName);
  ...
  if (!hasParams) {
    ...
    const withBase = s.applyBaseUrl(targetLocale, finalPath);
    const pathStr =
      typeof withBase === "string" ? withBase : ((withBase as RouteLike).path ?? finalPath);
