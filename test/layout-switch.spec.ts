...
test.describe("Layout Switch and Cookie Redirect", () => {
  test.describe("$switchLocale - layout translations", () => {
    test("layout text should update when switching locale via $switchLocale", async ({
      page,
      goto,
    }) => {
    ...
    test("layout should update on direct URL navigation to different locale", async ({
      page,
      goto,
    }) => {
      // Start with Chinese
  ...
  test.describe("Cookie-based redirect", () => {
    ...
    test("cookie should take priority over Accept-Language header", async ({
      page,
      goto,
      baseURL,
    }) => {
