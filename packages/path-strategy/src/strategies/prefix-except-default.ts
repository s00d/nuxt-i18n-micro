...
export class PrefixExceptDefaultPathStrategy extends BasePathStrategy {
  ...
  switchLocaleRoute(
    ...
    if (needsPrefix) {
      const found = findLocalizedRouteName(
        this.ctx.router,
        this.getLocalizedRouteNamePrefix(),
        baseName,
        toLocale,
      );
    ...
    const pathWithoutLocale = route.path?.startsWith(localePrefix)
  ...
  resolveLocaleRoute(
    ...
    const hasParams =
      sourceRoute.params !== null && hasKeys(sourceRoute.params as Record<string, unknown>);

    if (!hasGR) {
      return this.resolveLocaleRouteSimple(
        targetLocale,
        inputName,
        sourceRoute,
        resolved,
    ...
    return this.resolveLocaleRouteFull(
      targetLocale,
      inputName,
      sourceRoute,
      resolved,
  ...
  private resolveLocaleRouteSimple(
    ...
    if (inputName && hasParams) {
      if (!needsPrefix && this.ctx.router.hasRoute(inputName)) {
        const res = this.ctx.router.resolve({
          name: inputName,
      ...
      if (routeWithParams !== null)
        return preserveQueryAndHash(this.applyBaseUrl(targetLocale, routeWithParams), sourceRoute);
    }
    ...
    if (resolved.name !== null && resolved.path && resolved.path !== "/") {
      ...
      if (pathWithoutLocale && pathWithoutLocale !== "/") {
        const targetName = baseRouteName
          ? needsPrefix
            ? this.buildLocalizedName(baseRouteName, targetLocale)
            : baseRouteName
          : undefined;
        const pathForLocale = needsPrefix
        ...
        if (resolved.params || sourceRoute.params)
          route.params =
            resolved.params !== sourceRoute.params
              ? Object.assign({}, resolved.params, sourceRoute.params)
              : resolved.params;
        if (resolved.query || sourceRoute.query)
          route.query =
            resolved.query !== sourceRoute.query
              ? Object.assign({}, resolved.query, sourceRoute.query)
              : resolved.query;
    ...
    if (inputName && !hasParams) {
      if (!needsPrefix) {
        ...
        const routeByLocalizedName = tryResolveByLocalizedName(
          this.ctx.router,
          prefix,
          inputName,
          targetLocale,
        ...
        if (routeByLocalizedName !== null)
          return preserveQueryAndHash(
            this.applyBaseUrl(targetLocale, routeByLocalizedName),
            sourceRoute,
          );
      }
    }

    return this.resolveLocaleRouteFallback(
      targetLocale,
      resolved,
      sourceRoute,
      needsPrefix,
  ...
  private resolveLocaleRouteFull(
    ...
    if (inputName) {
      ...
      if (customBySynthetic !== null) {
        return this.buildNestedCustomResult(
          customBySynthetic,
          inputName,
          targetLocale,
          needsPrefix,
    ...
    if (resolved.name !== null) {
      ...
      if (unlocalizedPath !== null)
        return preserveQueryAndHash(this.applyBaseUrl(targetLocale, unlocalizedPath), sourceRoute);
      ...
      if (resolved.path && resolved.path !== "/" && resolved.name) {
        ...
        if (nestedInfo && pathWithoutLocale && pathWithoutLocale !== "/") {
          const parentPath = this.getParentPathForTarget(
            nestedInfo.parentKey,
            nestedInfo.keyWithSlash,
            targetLocale,
            currentRoute,
          );
          const segment = lastPathSegment(pathWithoutLocale);
          pathToUse = parentPath
            ? joinUrl(parentPath, segment)
            : pathWithoutLocale !== "/"
              ? pathWithoutLocale
              : null;
        } else {
  ...
  private resolveLocaleRouteFallback(
    ...
    const pathStr =
      typeof withBase === "string" ? withBase : ((withBase as RouteLike).path ?? pathForLocale);
  ...
  getRedirect(currentPath: string, detectedLocale: string): string | null {
    ...
    if (
      localeFromPath !== null &&
      isUnlocalizedRoute(pathWithoutLocale, this.ctx.globalLocaleRoutes)
    ) {
  ...
  ): { parentKey: string; keyWithSlash: string } | null {
    const gr = this.ctx.globalLocaleRoutes;
    if (!gr) return null;
  ...
  private buildNestedCustomResult(
    ...
    if (!needsPrefix)
      return preserveQueryAndHash(this.applyBaseUrl(targetLocale, pathNorm), sourceRoute);
    return preserveQueryAndHash(
      this.applyBaseUrl(targetLocale, joinUrl(targetLocale, pathNorm)),
      sourceRoute,
    );
  }
  ...
  private getParentPathForTarget(
    ...
    const parentRules =
      parentKey &&
      gr?.[parentKey] &&
      typeof gr[parentKey] === "object" &&
      !Array.isArray(gr[parentKey])
  ...
  getClientRedirect(currentPath: string, preferredLocale: string): string | null {
    ...
    if (needsPrefix) {
      targetPath = cleanDoubleSlashes(
        `/${preferredLocale}${customPath.startsWith("/") ? customPath : `/${customPath}`}`,
      );
    } else {
