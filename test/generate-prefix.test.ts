...
describe("nuxi generate with prefix strategy", () => {
  ...
  it("generates existing and nested child page routes for all locales", async () => {
    ...
    for (const parts of routeMatrix) {
      ...
      if (!existsSync(routeIndexPath)) throw new Error(`missing route: ${routeIndexPath}`);
  ...
  it("prerenders old and nested page locale payload routes", async () => {
    ...
    for (const [pageName, locale, key] of payloadMatrix) {
      ...
      if (!existsSync(payloadPath)) throw new Error(`missing payload: ${payloadPath}`);
      expect(existsSync(payloadPath)).toBe(true);
      const payload = JSON.parse(readFileSync(payloadPath, "utf-8")) as Record<string, string>;
      if (payload[key] === undefined) throw new Error(`missing key "${key}" in ${payloadPath}`);
