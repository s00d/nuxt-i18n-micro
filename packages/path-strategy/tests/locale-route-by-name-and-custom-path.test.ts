...
describe("localeRoute by route name + globalLocaleRoutes (locale-test style)", () => {
  ...
  test('prefix: route name "locale-test" with globalLocaleRoutes returns custom path with locale prefix', () => {
    ...
    router.resolve = (to: RouteLike | string) => {
      const r = (
        typeof to === "string"
          ...
          : {
              name: (to as RouteLike).name,
              path: `/${(to as RouteLike).name}`,
              fullPath: `/${(to as RouteLike).name}`,
            }
    ...
    expect(strategy.localeRoute("en", { name: "locale-test" }, currentRouteEn).path).toBe(
      "/en/locale-test-en",
    );
    expect(strategy.localeRoute("de", { name: "locale-test" }, currentRouteEn).path).toBe(
      "/de/locale-page-modify",
    );
    expect(strategy.localeRoute("ru", { name: "locale-test" }, currentRouteEn).path).toBe(
      "/ru/locale-page-modify-ru",
    );
  });
  ...
  test('prefix_and_default: route name "locale-test" with globalLocaleRoutes returns custom path with locale prefix', () => {
    ...
    const strategy = createPathStrategy(
      makeCtxAny("prefix_and_default", { globalLocaleRoutes, router }),
    );
    expect(strategy.localeRoute("en", { name: "locale-test" }, currentRouteEn).path).toBe(
      "/en/locale-test-en",
    );
