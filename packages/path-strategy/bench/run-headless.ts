...
async function runForBrowser(
  name: BrowserName,
  url: string,
): Promise<{ results: BenchResult[]; regressionCount: number }> {
  const sep = "=".repeat(60);
  console.log(`\n${sep}`);
  ...
  try {
    ...
    const results: BenchResult[] = await page.evaluate(() => {
      return new Promise<BenchResult[]>((resolve) => {
        const { results: r } = (
          window as { runBenchmarks: () => { results: BenchResult[] } }
        ).runBenchmarks();
        resolve(r);
      });
    });
...
function printSection(title: string, rows: BenchResult[], baseline: Baseline | null): void {
  ...
  if (hasBaseline) {
    ...
    lines.push(
      `\u2551 ${pad("Function", NW)}${padL("ops/ms", CW)}${padL("ms", CW)}${" ".repeat(CW + 4)}\u2551`,
    );
  }
  lines.push(`\u2551${"\u2500".repeat(W - 2)}\u2551`);
  ...
  for (const r of rows) {
    ...
    if (hasBaseline && prev && Number.isFinite(prev.opsPerMs) && prev.opsPerMs > 0) {
      ...
      lines.push(
        `\u2551 ${pad(r.name, NW)}${padL(r.opsPerMs.toFixed(1), CW)}${padL(r.totalMs.toFixed(2), CW)}${" ".repeat(CW + 4)}\u2551`,
      );
    }
  }
