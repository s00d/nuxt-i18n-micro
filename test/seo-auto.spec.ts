...
test.describe("SEO with dynamic metaBaseUrl (undefined)", () => {
  ...
  test("SSR: two different domains produce different canonical for the same path", async ({
    request,
  }) => {
    ...
    expect(html1).toMatch(
      /<link[^>]*rel="canonical"[^>]*href="https:\/\/site-a\.example\.com\/en\/about"/,
    );
    expect(html1).toMatch(
      /<meta[^>]*property="og:url"[^>]*content="https:\/\/site-a\.example\.com\/en\/about"/,
    );
    ...
    expect(html2).toMatch(
      /<link[^>]*rel="canonical"[^>]*href="https:\/\/site-b\.example\.com\/en\/about"/,
    );
    expect(html2).toMatch(
      /<meta[^>]*property="og:url"[^>]*content="https:\/\/site-b\.example\.com\/en\/about"/,
    );
  });
  ...
  test("SSR: without X-Forwarded-Host falls back to actual server host", async ({ request }) => {
    ...
    expect(html).not.toMatch(/content="auto/);
    expect(html).toMatch(
      /<link[^>]*rel="canonical"[^>]*href="http:\/\/(localhost|127\.0\.0\.1):\d+\/en"/,
    );
  });

  // ── Client: emulate a real domain in the browser ──

  test("client: after hydration canonical and og:url use emulated domain", async ({
    page,
    baseURL,
  }) => {
  ...
  test("client: x-default hreflang link uses emulated domain and points to default locale", async ({
    page,
    baseURL,
  }) => {
  ...
  test("client: canonical updates to correct path after SPA navigation", async ({
    page,
    baseURL,
  }) => {
  ...
  test("client: locale switch updates og:locale and keeps emulated domain in canonical", async ({
    page,
    baseURL,
  }) => {
  ...
  test("client: html lang and dir update on locale change with emulated domain", async ({
    page,
    baseURL,
  }) => {
