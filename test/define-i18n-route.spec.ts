...
test.describe("$defineI18nRoute behavior", () => {
  test("should generate alternates only for specified locales in $defineI18nRoute", async ({
    page,
    goto,
  }) => {
    ...
    const hreflangs = await Promise.all(
      Array.from({ length: count }).map((_, i) => alternateLinks.nth(i).getAttribute("hreflang")),
    );
  ...
  test("should generate correct canonical URL for restricted locale page", async ({
    page,
    goto,
  }) => {
    await goto("/test", { waitUntil: "domcontentloaded" });
  ...
  test("should generate correct og:locale meta for restricted locale page", async ({
    page,
    goto,
  }) => {
    await goto("/test", { waitUntil: "domcontentloaded" });
  ...
  test("should generate correct og:locale:alternate meta only for specified locales", async ({
    page,
    goto,
  }) => {
    await goto("/test", { waitUntil: "domcontentloaded" });
  ...
  test("should not generate alternates for non-specified locales in build output", async ({
    page,
    goto,
  }) => {
    await goto("/test", { waitUntil: "domcontentloaded" });
  ...
  test("should generate x-default hreflang pointing to the default locale URL", async ({
    page,
    goto,
    baseURL,
  }) => {
...
test.describe("$setI18nRouteParams behavior", () => {
  test("should use correct slug from $setI18nRouteParams when switching from English to Spanish", async ({
    page,
    goto,
  }) => {
  ...
  test("should use correct slug from $setI18nRouteParams when switching from Spanish to English", async ({
    page,
    goto,
  }) => {
...
test.describe("Product index page with localeRoutes", () => {
  test("should be accessible via localized routes", async ({ page, goto }) => {
    ...
    await expect(
      page.getByText(
        "Discover our collection of high-quality products designed to meet your needs and exceed your expectations.",
      ),
    ).toBeVisible();
    ...
    await expect(
      page.getByText(
        "Descubra nuestra colección de productos de alta calidad diseñados para satisfacer sus necesidades y superar sus expectativas.",
      ),
    ).toBeVisible();
  });
  ...
  test("should redirect from /product to /our-products for default locale", async ({
    page,
    goto,
  }) => {
  ...
  test("should have correct meta tags for English product page", async ({
    page,
    goto,
    baseURL,
  }) => {
    ...
    await expect(page.locator("meta#i18n-og-url")).toHaveAttribute(
      "content",
      `${normalizedBaseURL}/our-products`,
    );
    ...
    await expect(page.locator("link#i18n-alternate-es")).toHaveAttribute(
      "href",
      `${normalizedBaseURL}/es/nuestros-productos`,
    );
    await expect(page.locator("link#i18n-alternate-es-ES")).toHaveAttribute(
      "href",
      `${normalizedBaseURL}/es/nuestros-productos`,
    );
  });

  test("should have correct meta tags for Spanish product page", async ({
    page,
    goto,
    baseURL,
  }) => {
...
test.describe("Product detail page with localeRoutes", () => {
  test("should have correct meta tags for English product detail page", async ({
    page,
    goto,
    baseURL,
  }) => {
    ...
    await expect(page.locator("meta#i18n-og-url")).toHaveAttribute(
      "content",
      `${normalizedBaseURL}/our-products/coffee-filter-en`,
    );
    await expect(page.locator("meta#i18n-og-alt-es-ES")).toHaveAttribute("content", "es-ES");
  ...
  test("should have correct meta tags for Spanish product detail page", async ({
    page,
    goto,
    baseURL,
  }) => {
