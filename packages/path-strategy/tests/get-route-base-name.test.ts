...
describe("getRouteBaseName (utility)", () => {
  test("strips localized prefix and locale suffix", () => {
    const result = {
      localizedAboutEn: getRouteBaseName(
  ...
  test("does not strip -en when it is part of a word (product-screen)", () => {
    const result = {
      localizedProductScreenEn: getRouteBaseName(
        { name: "localized-product-screen-en" },
        { locales },
      ),
...
describe("getRouteBaseName (strategy method)", () => {
  test("PrefixExceptDefaultPathStrategy returns base name", () => {
    const strategy = new PrefixExceptDefaultPathStrategy(
      makePathStrategyContext(baseConfig, "prefix_except_default"),
    );
