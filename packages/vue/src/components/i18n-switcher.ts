...
export const I18nSwitcher = defineComponent({
  ...
  props: {
    ...
    localeRoute: {
      type: Function as PropType<
        (to: string | { path?: string }, locale?: string) => string | { path?: string }
      >,
      default: undefined,
    },
  },
  setup(props, { slots }) {
    ...
    if (!i18n) {
      throw new Error(
        "[i18n-micro] I18nSwitcher: i18n instance not found. Make sure i18n plugin is installed.",
      );
    }
    ...
    const currentLocale = computed(() => {
      if (props.currentLocale !== undefined) {
        return typeof props.currentLocale === "function"
          ? props.currentLocale()
          : props.currentLocale;
      }
      ...
      if (dropdownOpen.value) {
        ...
        locales.value.forEach((localeItem) => {
          ...
          if (slots["before-link-content"]) {
            linkContent.push(
              ...normalizeSlot(slots["before-link-content"]({ locale: localeItem })),
            );
          }
