...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  const switchLocaleLogic = (
    toLocale: string,
    i18nParams: I18nRouteParams,
    ...
    const switchedRoute = i18nStrategy.switchLocaleRoute(
      fromLocale,
      toLocale,
      toResolvedRouteLike(resolvedTarget),
      {
    ...
    if (
      typeof switchedRoute === "string" &&
      (switchedRoute.startsWith("http://") || switchedRoute.startsWith("https://"))
    ) {
  ...
  // === 4. INITIALIZATION ===
  const serverLocale = import.meta.server
    ? nuxtApp.ssrContext?.event?.context?.i18n?.locale
    : undefined;
  ...
  const initialRouteName = getPluginRouteName(
    toResolvedRouteLike(router.currentRoute.value),
    initialLocale,
  );
  ...
  const tFast = (
    ...
    if (val === undefined) {
      if (customMissingHandler.value) {
        ...
        console.warn(
          `[i18n] Missing key '${key}' in '${currentLocale}' for route '${currentRouteName}'`,
        );
      }
  ...
  const provideData = {
    ...
    getLocaleName: () =>
      i18nStrategy.getCurrentLocaleName(
        toResolvedRouteLike(router.currentRoute.value),
        getLocale() ?? null,
      ),
    ...
    tc: (key: string, params: number | Params, defaultValue?: string): string => {
      ...
      return (
        (plural(
          key,
          Number.parseInt(count.toString(), 10),
          _params,
          currentLocale,
          tFast,
        ) as string) ??
    ...
    switchRoute: (
      route: RouteLocationNamedRaw | RouteLocationResolvedGeneric | string,
      toLocale?: string,
    ) => {
    ...
    localeRoute(
      ...
      const targetLocale =
        locale !== undefined && locale !== "" ? String(locale) : getCurrentLocale();
      ...
      if (result.query && Object.keys(result.query).length)
        out.query = result.query as Record<string, string>;
...
export interface PluginsInjections {
  ...
  $getRouteName: (
    route?: RouteLocationNamedRaw | RouteLocationResolvedGeneric,
    locale?: string,
  ) => string;
  ...
  $_ts: (
    route: RouteLocationNormalizedLoaded,
  ) => (key: string, params?: Params, defaultValue?: string | null) => string;
  $tc: (key: string, params: number | Params, defaultValue?: string) => string;
