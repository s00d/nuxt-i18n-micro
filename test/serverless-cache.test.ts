...
describe("serverless caching emulation (FS driver for serialization check)", () => {
  ...
  it("4. Version Change / Cleanup: clears old cache when build version changes", async () => {
    ...
    // ВАЖНО: в setup() dateBuild берется из `options.dateBuild ?? Date.now()`.
  ...
  it("7. Concurrency / Race Conditions: handles parallel cold requests", async () => {
    ...
    expect(
      firstResult && firstResult.status === "ok" ? firstResult.data.hello : "Hello World",
    ).toBe("Hello World");
    ...
    if (existsSync(cacheDir)) {
      try {
        ...
        const enFiles = fileList.filter(
          (f: unknown) => typeof f === "string" && f.includes("index") && f.includes("en"),
        );
